...
function bodyLoad() { 
	alert('${name} ได้เปลี่ยนสถานะเป็น ผู้แทนรุ่น เรียบร้อยแล้ว')
	window.close();
	}
...
<body onload="bodyLoad();">
  ...
  <form action="" target="">
    <fieldset>
      <div class="widget">
        <div class="head">
          <h5 class="iCoverflow">เปลี่ยนสถานะเป็นผู้แทนรุ่นสำเร็จแล้ว</h5>
          <br>
        </div>
        <div class="rowElem noborder">
          <label>ชื่อ - นามสกุล:</label>
          <div class="formRight">${name}</div>
          <div class="fix"></div>
        </div>
        <div class="rowElem">
          <label>รหัสนักศึกษา:</label>
          <div class="formRight">${studentID}</div>
          <div class="fix"></div>
        </div>
        <button  type="button" value="close" class="greyishBtn submitForm" onclick="window.close();">Close</button>
        <button  type="button" NAME="btnPopup_1" ID="btnPopup_1" value="view" class="greyishBtn submitForm" OnClick="Profile(${id})">ดูข้อมูลประวัติ</button>
        <div class="fix"></div>
      </div>
    </fieldset>
  </form>
